# Hi, I'm Zikai Liu 👋

### Full-Stack · AI Agent · Cloud-Native Engineer

📍 Shenzhen, China · 😄 You can call me KK

I design and build production-oriented applications across **Web, Mobile, Desktop, AI, and Cloud-Native Infrastructure**.

I have end-to-end engineering capabilities spanning product prototyping, frontend and backend development, data modeling, AI integration, containerized delivery, Kubernetes orchestration, and production observability.

My current focus includes:

- 🤖 AI Agents, RAG, tool calling, and multi-agent collaboration
- 🌐 Full-stack Web, Mobile, and Desktop application architecture
- ⚡ High-performance APIs, asynchronous tasks, and event-driven systems
- ☸️ Docker and Kubernetes-based cloud-native platforms
- 🔍 Production observability through metrics, logs, and monitoring
- 🎬 AI-powered audio, video, OCR, and computer vision workflows

---

## 🚀 Core Engineering Capabilities

### 🤖 AI Agents & Intelligent Applications

- LLM application and AI Agent architecture
- Retrieval-Augmented Generation and knowledge base systems
- Tool calling, workflow orchestration, and multi-agent collaboration
- Speech recognition, speech synthesis, and audio processing
- OCR, computer vision, and generative media pipelines
- AI model serving, API integration, and containerized deployment

### 🌐 Full-Stack Application Development

- React, Next.js, and TypeScript frontend architecture
- Backend services built with FastAPI, Django, Gin, and Spring Boot
- RESTful APIs, authentication, authorization, and state management
- Cross-platform Mobile and Desktop application development
- Relational, document, cache, and search-oriented data architecture

### ☸️ Kubernetes & Cloud-Native Engineering

- Docker image building and multi-environment delivery
- Kubernetes workload deployment and orchestration
- Deployment, Service, and Ingress configuration
- ConfigMap, Secret, and environment configuration management
- Health checks, resource limits, and horizontal autoscaling
- Rolling updates, fault recovery, and high-availability practices
- Reverse proxy and TLS termination with Nginx and Caddy
- Observability with Prometheus, Grafana, and the ELK Stack

### ⚡ Distributed Systems & Engineering

- Event-driven architecture with Kafka
- Asynchronous task processing with Celery
- Redis caching, distributed state, and performance optimization
- Service discovery and coordination with Consul and etcd
- Search and log analytics with Elasticsearch
- Metrics collection and visualization with Prometheus and Grafana

---

## 🧰 Complete Technology Matrix

### 🤖 AI Stack

![LangChain](https://badgen.net/badge/LangChain/LLM/6DB33F)
![Dify](https://badgen.net/badge/Dify/AI/6DB33F)
![ComfyUI](https://badgen.net/badge/ComfyUI/GenAI/9B59B6)
![Whisper](https://badgen.net/badge/Whisper/ASR/5E2B97)
![GPT-SoVITS](https://badgen.net/badge/GPT-SoVITS/TTS/9B59B6)
![PaddleOCR](https://badgen.net/badge/PaddleOCR/OCR/2E6BE6)

**Focus Areas**

`LLM Applications` · `AI Agents` · `RAG` · `Tool Calling` · `Workflow Orchestration` · `ASR` · `TTS` · `OCR` · `Generative AI`

---

### 🧠 Programming Languages

![Python](https://badgen.net/badge/Python/Expert/3776AB)
![JavaScript](https://badgen.net/badge/JavaScript/Advanced/F7DF1E)
![TypeScript](https://badgen.net/badge/TypeScript/Advanced/3178C6)
![Java](https://badgen.net/badge/Java/Advanced/ED8B00)
![Go](https://badgen.net/badge/Go/Advanced/00ADD8)
![C](https://badgen.net/badge/C/Advanced/A8B9CC)
![Shell](https://badgen.net/badge/Shell/Advanced/4EAA25)

---

### 🌐 Web Frontend

![React](https://badgen.net/badge/React/Advanced/61DAFB)
![Redux](https://badgen.net/badge/Redux/State/764ABC)
![React Router](https://badgen.net/badge/React%20Router/Routing/CA4245)
![Next.js](https://badgen.net/badge/Next.js/SSR/000000)

#### 🎨 UI & Design Systems

![TailwindCSS](https://badgen.net/badge/TailwindCSS/UI/06B6D4)
![Ant Design](https://badgen.net/badge/AntDesign/UI/0170FE)
![Shadcn/UI](https://badgen.net/badge/Shadcn%2FUI/UI/111111)

#### ⚙️ HTTP & Build Tooling

![Axios](https://badgen.net/badge/Axios/HTTP/5A29E4)
![Vite](https://badgen.net/badge/Vite/Fast/646CFF)

---

### 🖥️ Mobile & Desktop

![Flutter](https://badgen.net/badge/Flutter/Mobile%20%2B%20Desktop/02569B)
![React Native](https://badgen.net/badge/React%20Native/Mobile/61DAFB)
![Electron](https://badgen.net/badge/Electron/Desktop/47848F)

**Application Targets**

`Android` · `iOS` · `Windows` · `macOS` · `Linux`

---

### 🧩 Backend & Service Frameworks

![Django](https://badgen.net/badge/Django/Advanced/092E20)
![Django REST](https://badgen.net/badge/Django-REST/API/FF884D)
![Flask](https://badgen.net/badge/Flask/Micro/000000)
![FastAPI](https://badgen.net/badge/FastAPI/Modern/009688)
![Gin](https://badgen.net/badge/Gin/Go/00ADD8)
![GORM](https://badgen.net/badge/GORM/ORM/00ADD8)
![Spring Boot](https://badgen.net/badge/SpringBoot/Java/6DB33F)
![Tomcat](https://badgen.net/badge/Tomcat/Servlet/F8DC75)
![Gunicorn](https://badgen.net/badge/Gunicorn/WSGI/499848)
![uWSGI](https://badgen.net/badge/uWSGI/Server/DA3B8A)
![Uvicorn](https://badgen.net/badge/Uvicorn/ASGI/009688)

**Architecture Capabilities**

`RESTful APIs` · `Microservices` · `ASGI` · `WSGI` · `ORM` · `Authentication` · `Authorization`

---

### 🗄️ Databases, Cache & Distributed Storage

![MySQL](https://badgen.net/badge/MySQL/Primary/00758F)
![PostgreSQL](https://badgen.net/badge/PostgreSQL/Primary/4169E1)
![MongoDB](https://badgen.net/badge/MongoDB/NoSQL/47A248)
![Redis](https://badgen.net/badge/Redis/Cache/DC382D)
![etcd](https://badgen.net/badge/etcd/KeyValue/419EDA)

**Data Capabilities**

`Relational Databases` · `Document Databases` · `Caching` · `Key-Value Storage` · `Data Modeling`

---

### 📨 Messaging, Streaming & Task Queues

![Kafka](https://badgen.net/badge/Kafka/Streaming/231F20)
![Celery](https://badgen.net/badge/Celery/TaskQueue/37814A)
![Flower](https://badgen.net/badge/Flower/Monitor/DA3B8A)
![Consul](https://badgen.net/badge/Consul/Service/F24C53)

**System Capabilities**

`Event-Driven Architecture` · `Message Streaming` · `Task Queues` · `Service Discovery`

---

### 🔎 Search, Logging & Observability

![Elasticsearch](https://badgen.net/badge/Elasticsearch/Search/005571)
![Logstash](https://badgen.net/badge/Logstash/ETL/005571)
![Kibana](https://badgen.net/badge/Kibana/Explore/005571)
![Grafana](https://badgen.net/badge/Grafana/Metrics/F46800)
![Prometheus](https://badgen.net/badge/Prometheus/Monitoring/E6522C)

**Observability Capabilities**

`Metrics` · `Logging` · `Search` · `Visualization` · `Monitoring` · `Alerting`

---

### ☁️ Cloud-Native Infrastructure & DevOps

![Kubernetes](https://badgen.net/badge/Kubernetes/Orchestration/326CE5)
![Docker](https://badgen.net/badge/Docker/Containers/2496ED)
![Nginx](https://badgen.net/badge/Nginx/Web/009639)
![Caddy](https://badgen.net/badge/Caddy/AutoTLS/1F88C0)
![Ansible](https://badgen.net/badge/Ansible/Automation/EE0000)
![Git](https://badgen.net/badge/Git/VCS/F05032)

#### ☸️ Kubernetes Engineering

![Deployment](https://badgen.net/badge/K8s/Deployment/326CE5)
![Service](https://badgen.net/badge/K8s/Service/326CE5)
![Ingress](https://badgen.net/badge/K8s/Ingress/326CE5)
![ConfigMap](https://badgen.net/badge/K8s/ConfigMap/326CE5)
![Secret](https://badgen.net/badge/K8s/Secret/326CE5)
![HPA](https://badgen.net/badge/K8s/HPA/326CE5)
![Rolling Update](https://badgen.net/badge/K8s/Rolling%20Update/326CE5)

**Cloud-Native Capabilities**

`Containerization` · `Orchestration` · `Service Exposure` · `Autoscaling` · `Rolling Deployments` · `Infrastructure Automation`

---

### 🧪 QA, Crawling & Automation

![Scrapy](https://badgen.net/badge/Scrapy/Crawler/60A839)
![Selenium](https://badgen.net/badge/Selenium/Browser/43B02A)
![ZAP](https://badgen.net/badge/ZAP/Security/00549E)
![PyQt](https://badgen.net/badge/PyQt/GUI/41CD52)

**Engineering Areas**

`Web Crawling` · `Browser Automation` · `Security Testing` · `Desktop Automation`

---

### 📊 Data, Vision & Media

![Pandas](https://badgen.net/badge/Pandas/Data/150458)
![OpenCV](https://badgen.net/badge/OpenCV/Vision/5C3EE8)
![FFmpeg](https://badgen.net/badge/FFmpeg/Media/007808)
![MoviePy](https://badgen.net/badge/MoviePy/Video/FF5733)

**Processing Capabilities**

`Data Processing` · `Computer Vision` · `Audio Processing` · `Video Processing`

---

## 🔭 Current Focus

- Building AI Agents with reliable tool-use capabilities
- Exploring multi-agent collaboration and task orchestration
- Designing production-oriented RAG and knowledge retrieval systems
- Running AI services and distributed applications on Kubernetes
- Improving autoscaling, monitoring, and fault recovery
- Connecting the complete lifecycle from AI development to production delivery

---

## 💡 Engineering Philosophy

```text
Design for clarity.
Build for reliability.
Observe before optimizing.
Automate repeated work.
Create systems that remain maintainable, scalable, and evolvable.
```

---

## 📫 Connect With Me

- 📧 Email: [zikailiu787@gmail.com](mailto:zikailiu787@gmail.com)
- 🧑‍💻 GitHub: [@Liuzikai](https://github.com/Liuzikai)
- 📍 Location: Shenzhen, China
- 😄 You can call me KK

---

> Building intelligent software from user experience and business services to Kubernetes infrastructure.
